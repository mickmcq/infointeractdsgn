...
You worked through the first four stages in our first class and ten of you turned them in. (I'd like to see more.) Here are a few comments about them.
...
```{r}
#| engine: tikz
#| label: fig-triangle-roles
#| echo: false
#| fig-ext: svg
#| out-width: "55%"
#| fig-align: center
#| fig-alt: "triangle with vertices: psych, coding, and graphics; internal nodes are occupation titles"
#| engine.opts: !expr list(template = "tikzTemplate.tex", dvisvgm.opts = c(paste0("--libgs=", Sys.getenv("LIBGS", unset = "/opt/homebrew/lib/libgs.dylib")), "--font-format=woff", "--exact-bbox"))
\input{fiTriangle.tex}
```
...
A blogger variously named Min Ming Lo or Lo Min Ming has written a couple of articles trying to distinguish between these roles. These have appeared at links that have since disappeared, but he still has a site at [http://minming.net/](http://minming.net/).
...
The vertices of the triangle represent emphases or influences. The job titles are positioned to reflect the strength of the emphases. For example, graphics is the strongest influence on a visual designer. The product designer is positioned closest to the center because it is often the catch-all for all these positions. Note that there is no *information designer* in the picture nor in the Lo articles. If I were to place information designer on this graphic it would be midway between graphics and coding, just below product designer. This is a rarely used job title, though. *Visual analytics specialist* might be what people think of instead of information designer. It also may be a synonym for information architect in some minds.
:::

## Charting an example learning journey

```{r}
#| engine: tikz
#| label: fig-triangle-ambition
#| echo: false
#| fig-ext: svg
#| out-width: "55%"
#| fig-align: center
#| engine.opts: !expr list(template = "tikzTemplate.tex", dvisvgm.opts = c(paste0("--libgs=", Sys.getenv("LIBGS", unset = "/opt/homebrew/lib/libgs.dylib")), "--font-format=woff", "--exact-bbox"))
\input{fiTriangleA.tex}
```

::: {.notes}
Here is the same triangle with the job titles stripped away, marked up the way I am going to ask you to mark up your own. This person places themselves near the psych vertex now and wants to end up in the middle, which is to say they want to become a product designer.
:::

# People

## Rudolph Arnheim
Major definer of artistic principles via his most famous book, *Art and visual perception*, @Arnheim1974. His most important principle is perhaps directed tension, described in Chapter 9 of this essential text.
...
## Matias Duarte
Former head of design for Android, and spokesperson for Material Design. Now VP of Google.
...
::: {.notes}
There is a vast amount of scholarship on link rot, some of it in the legal arena because of its impact on reading US Supreme Court opinions. Schemes to overcome link rot such as doi and URI have been proposed, developed, and ignored since the beginning of the web.
...
HCI academic programs in North America include
U Washington,
Berkeley,
Carnegie-Mellon,
U Colorado - Boulder,
...
JavaScript (via TypeScript) is the language of the web. It's bad news not to understand this ubiquitous language
...
::: {.notes}
You may use any language to manipulate JSON, including JavaScript, but understanding JSON and XML requires study separate from that of JavaScript. The inventor of JSON (who said that its pronunciation is *Jason*) proudly wrote the rules of JSON on a business card to emphasize its simplicity.
:::
...
*Hacker News* is my preferred news source. You should try to at least skim it weekly to improve your picture of information technology in general. It also makes sense to read Andrew Ng's weekly newsletters about AI, *The Batch* and *Data Points*, as well as Simon Willison's blog.
...
## Trends a few years later
![](fiTop10DesignTools2025.png){fig-alt="barchart of top tools of 2025, with Figma even more dominant than in 2022"}

::: {.notes}
This chart comes from [https://www.uxtools.co/survey/interface-design/overview](https://www.uxtools.co/survey/interface-design/overview)
:::

## Figma's IPO

![](figmaDebut.png){fig-alt="Figma's IPO news article"}

::: {.notes}
Figma's IPO was a big deal. Offered at 33USD, shares immediately sold for ~120USD. As of September 2025, the price has settled down to about 70USD. The company still doesn't face significant competition. Figma is certainly the flavor of the month for designers. What could dethrone it? AI? Better integration with code? You have to keep monitoring competitors and keep trying to use their new products to know when to shift.

Remember that no tool is likely to outlast your career! Your strongest skill is your adaptability. Get good at Figma but don't just be a Figma jockey. I have corporate clients who beg me to teach concepts rather than skills because they say that they can always outsource skills. They can hire a Figma jockey temporarily. The people they want to hire for keeps are people with ideas. So the other question besides how to be adaptable is how to have ideas. Nobel Prize winner Linus Pauling said that the best way to have a good idea is to have a lot of ideas.
:::

## Figma's stock price
![](fiFigmaStockMax.png){fig-alt="Figma's stock opened at 130 in July 2025 and slid to 29 by Aug 2026"}
...
A historical resource for HCI literature may be found at [hcibib](http://www.hcibib.org).  You may find it helpful for some of your work.
...
## First, let's look at the triangles
- See if you can find patterns
- See if you can find people like you
- Keep in mind that you will be colleagues for a long time to come!
  - What does that mean?
  - How does that affect your interactions?

## Now, a Figma exercise
- animate the transition on the following slide
- There are vastly many ways to do it
- If you can, allow the user to go back to the initial state
- Yes, this is a test, but no, it won't be graded
- We need to know what you already know about Figma to prepare appropriate lessons

## Animate this transition
![](fiOpenClose.png){fig-alt="Two screens, the first with the search box closed; the second with it open"}
